...
//==============================================================================================================
export const candidataDadosGerais = async (req, res) => {
  try {
    // obtém da tabela de candidatas todos os registros
    ...
    res.status(200).json({menor, maior, media: Number(media).toFixed(1), num})
  } catch (error) {
    res.status(400).json({ id: 0, msg: "Erro: " + error.message })
  }
}
...
export const candidataPorCidade = async (req, res) => {
  try {
    // obtém da tabela de candidatas todos os registros
    const consulta = await dbKnex("candidatas").select("cidade")
                    .count({num: "*"}).groupBy("cidade")
...
//===================================================================================================================
export const candidataVotos = async (req, res) => {
  try {
    // obtém da tabela de candidatas todos os registros
    const candidatas = await dbKnex.select('nome','votos')
                    .from("candidatas").orderBy("votos", "desc")
                    .limit()
    res.status(200).json(candidatas)
  } catch (error) {
    res.status(400).json({ id: 0, msg: "Erro: " + error.message })
